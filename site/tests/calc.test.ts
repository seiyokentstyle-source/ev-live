...
describe("AT間モデル（hitsに投入G0=6要素目・差枚時給・OUT/IN機械割）", () => {
  ...
  const bet = 3;
  const use = 1.53;
  // [台番号, 取得日, 初当りG, 総獲得, 道中CZ, 投入G0]
  // 投入G0 = 初当りG + 引き戻しゾーンG + やめtail（即やめなら初当りG）。
  const hits: EvSamples["hits"] = [
    ["101", "2026-07-01", 300, 700, 0, 366], // 単発+66やめtail
    ["102", "2026-07-02", 400, 900, 0, 466],
    ["103", "2026-07-03", 200, 300, 0, 200] // RB単発（即やめ・tail0）
  ];
  ...
  test("投資は投入G0基準（やめtail込み）＝初当りGより多い", () => {
    const anchors = computeAnchors(hits, [], calc, 21.74, kan, 1);
    const a0 = anchors.find((a) => a.g === 0)!;
    // 初当りGだけなら平均(300+400+200)/3=300、投入G0込みは(366+466+200)/3=344 → 投入も増える
    expect(a0.inv).toBe(Math.round(((366 + 466 + 200) * use) / 3));
    expect(a0.inv).toBeGreaterThan(Math.round(((300 + 400 + 200) * use) / 3));
  });
});
